...
Write-Log("Starting script to set gc mode")
Write-Log("Create the document if required")

# Create the document if required
Create-ConfigFileIfNotExists

Write-Log("Load the configuration file into the XML document")
...
{
	Write-Log("Config document not null")
